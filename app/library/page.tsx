      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <header className="mb-8">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-6 p-1 bg-slate-900/80 rounded-xl border border-slate-800 w-fit">
            ...
            >
              🕉️ Bhagavad Gita Cognitive Library (13 Anchors)
            </button>
            <button
              type="button"
        ...
        {/* VIEW 1: 100% Text-Only Bhagavad Gita Library */}
        {activeView === "gita" && (
          <GitaLibraryView
            onSelectVerse={(item) => {
              router.push(`/?focus=${item.id}&ref=${encodeURIComponent(item.reference_header)}`);
            }}
          />
        )}

        {/* VIEW 2: Clinical Neuropsychology & Breathwork Library */}
        {activeView === "conditions" && (
          <div>
...
export default function PsychologyLibraryPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-slate-950 text-slate-400 flex items-center justify-center font-mono text-sm">Loading Sanctuary Library...</div>}>
      <LibraryContent />
    </Suspense>
  );
}
